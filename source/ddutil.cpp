...
HRESULT CDisplay::CreateFullScreenDisplay( HWND hWnd, DWORD dwWidth,
                                           DWORD dwHeight, DWORD dwBPP, DWORD dwFRate )
{
    HRESULT hr;
    ...
    if( FAILED( m_pDD->SetDisplayMode( dwWidth, dwHeight, dwBPP, dwFRate, 0 ) ) )
        return E_FAIL;
...
HFONT CDisplay::CreateHFONT(const char *facename, int size, long lfweight, 
							bool bItalic, bool bUnderline, bool bStrikeOut)
{
	 LOGFONT logfont;    //要填妥LOGFONT這個資料結構
      
     strcpy(logfont.lfFaceName, facename);   //填入字型名稱
     logfont.lfWidth = 0;
     logfont.lfHeight = -size*16/12;    //字型大小的公式
     logfont.lfEscapement = 0;
     logfont.lfOrientation = 0;
     logfont.lfWeight = lfweight;   //不變粗
     logfont.lfItalic = bItalic;             //不要斜體
     logfont.lfUnderline = bUnderline;      //不要底線
     logfont.lfStrikeOut = bStrikeOut;      //不要刪除線
     logfont.lfCharSet = DEFAULT_CHARSET;            //預設的字集
     logfont.lfOutPrecision = OUT_CHARACTER_PRECIS; 
     logfont.lfClipPrecision = CLIP_DEFAULT_PRECIS;    
     logfont.lfQuality = DEFAULT_QUALITY;     
     logfont.lfPitchAndFamily = DEFAULT_PITCH;
      
     return  CreateFontIndirect(&logfont);   //產生字型
}
